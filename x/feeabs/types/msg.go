...
var _ sdk.Msg = &MsgInterchainQueryBalances{}

// Route Implements Msg.
func (m MsgInterchainQueryBalances) Route() string { return sdk.MsgTypeURL(&m) }

// Type Implements Msg.
func (m MsgInterchainQueryBalances) Type() string { return sdk.MsgTypeURL(&m) }

// GetSigners returns the expected signers for a MsgMintAndAllocateExp .
func (m MsgInterchainQueryBalances) GetSigners() []sdk.AccAddress {
	daoAccount, err := sdk.AccAddressFromBech32(m.FromAddress)
	if err != nil {
		panic(err)
	}
...
// GetSignBytes Implements Msg.
func (m MsgInterchainQueryBalances) GetSignBytes() []byte {
	return sdk.MustSortJSON(legacy.Cdc.MustMarshalJSON(&m))
}

// ValidateBasic does a sanity check on the provided data.
func (m MsgInterchainQueryBalances) ValidateBasic() error {
	_, err := sdk.AccAddressFromBech32(m.FromAddress)
	if err != nil {
		return sdkerrors.Wrap(err, "from address must be valid address")
	}
...
func NewMsgInterchainQueryBalances(fromAddr sdk.AccAddress, queryAddress string) *MsgInterchainQueryBalances {
	return &MsgInterchainQueryBalances{
		FromAddress:  fromAddr.String(),
