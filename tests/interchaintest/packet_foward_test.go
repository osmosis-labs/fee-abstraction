...
func TestPacketForwardMiddleware(t *testing.T) {
	...
	t.Run("xcs", func(t *testing.T) {
		...
		gaiaHeight, err = gaia.Height(ctx)
		require.NoError(t, err)
		dstAddress = feeabsUser.FormattedAddress()
		transfer = ibc.WalletAmount{
			Address: dstAddress,
			Denom:   gaia.Config().Denom,
			Amount:  amountToSend,
		...
		// // Instatiate
		owner := osmosisUser.FormattedAddress()
		...
		osmosisUserBalance, err := osmosis.GetBalance(ctx, osmosisUser.FormattedAddress(), uatomOnOsmosis)
		require.NoError(t, err)
		...
		osmosisUserBalance, err = osmosis.GetBalance(ctx, osmosisUser.FormattedAddress(), stakeOnOsmosis)
		require.NoError(t, err)
		require.Equal(t, amountToSend, osmosisUserBalance)
