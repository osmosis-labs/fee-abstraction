...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewFeeApp(log.NewNopLogger(), newDB, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
	require.Equal(t, "FeeApp", newApp.Name())
...
func setupSimulationApp(t *testing.T, msg string) (simtypes.Config, dbm.DB, simtestutil.AppOptionsMap, *FeeApp) {
	...
	app := NewFeeApp(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
	require.Equal(t, "FeeApp", app.Name())
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			db := dbm.NewMemDB()
			app := NewFeeApp(logger, db, nil, true, appOptions, interBlockCacheOpt(), baseapp.SetChainID(SimAppChainID))
