...
func setup(tb testing.TB, withGenesis bool, invCheckPeriod uint) (*FeeAbs, GenesisState) {
	...
	require.NoError(tb, err)
	baseAppOpts := []func(*baseapp.BaseApp){baseapp.SetSnapshot(snapshotStore, snapshottypes.SnapshotOptions{KeepRecent: 2})}
	db := dbm.NewMemDB()
	app := NewFeeAbs(
		log.NewNopLogger(),
...
func initAccountWithCoins(app *FeeAbs, ctx sdk.Context, addr sdk.AccAddress, coins sdk.Coins) {
