...
func setup(tb testing.TB, chainID string, withGenesis bool, invCheckPeriod uint) (*FeeApp, GenesisState) {
	...
	app := NewFeeApp(log.NewNopLogger(), db, nil, true, appOptions, bam.SetChainID(chainID), bam.SetSnapshot(snapshotStore, snapshottypes.SnapshotOptions{KeepRecent: 2}))
	if withGenesis {
...
func NewFeeAppWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *FeeApp {
	t.Helper()
	...
	app := NewFeeApp(options.Logger, options.DB, nil, true, options.AppOpts)
...
// Setup initializes a new FeeApp. A Nop logger is set in FeeApp.
func Setup(t *testing.T) *FeeApp {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new FeeApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the FeeApp from first genesis
// account. A Nop logger is set in FeeApp.
...
func SetupWithEmptyStore(tb testing.TB) *FeeApp {
	tb.Helper()
...
func GenesisStateWithSingleValidator(t *testing.T, app *FeeApp) GenesisState {
	t.Helper()
...
func AddTestAddrsIncremental(app *FeeApp, ctx sdk.Context, accNum int, accAmt sdkmath.Int) []sdk.AccAddress {
	return addTestAddrs(app, ctx, accNum, accAmt, simtestutil.CreateIncrementalAccounts)
}

func addTestAddrs(app *FeeApp, ctx sdk.Context, accNum int, accAmt sdkmath.Int, strategy simtestutil.GenerateAccountStrategy) []sdk.AccAddress {
...
func initAccountWithCoins(app *FeeApp, ctx sdk.Context, addr sdk.AccAddress, coins sdk.Coins) {
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewFeeApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, simtestutil.NewAppOptionsWithFlagHome(dir))
...
func AddHostZoneProposalFixture(mutators ...func(*feeabstypes.AddHostZoneProposal)) *feeabstypes.AddHostZoneProposal {
	p := &feeabstypes.AddHostZoneProposal{
		Title:           "Title",
