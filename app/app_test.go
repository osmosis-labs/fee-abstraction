...
func TestFeeAppdExport(t *testing.T) {
	...
	gapp := NewFeeAppWithCustomOptions(t, false, SetupOptions{
	...
	newGapp := NewFeeApp(logger, db, nil, true, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
	_, err = newGapp.ExportAppStateAndValidators(false, []string{}, nil)
	require.NoError(t, err, "ExportAppStateAndValidators should not have an error")
}

// TestBlockedAddrs ensure that blocked addresses are properly set in bank keeper
